...
class InvarHungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_var: float = 1, cost_op: float = 1):
        """Creates the matcher
        ...
        assert cost_var != 0 or cost_op != 0, "all costs cant be 0"
    ...
    def forward(self, outputs, targets):
        ...
        out_ops = outputs["pred_ops"].flatten(0, 1)  # [batch_size * num_queries, num_classes]
        out_eq = outputs["pred_eq"].flatten(0, 1)  # [batch_size * num_queries, bool]

        tgt_ops = targets["ops"].flatten(0, 1)  # [batch_size * num_queries, num_classes]
        tgt_eq = targets["eq"].flatten(0, 1)  # [batch_size * num_queries, bool]

        assert tgt_ops.shape == out_ops.shape
        assert tgt_eq.shape == out_eq.shape

        # Compute the classification cost. Contrary to the loss, we don't use the NLL,
        # but approximate it in 1 - proba[target class].
        # The 1 is a constant that doesn't change the matching, it can be ommitted.
        cost_eq = -1 * out_eq * tgt_eq  # [batch_size * num_queries, num_classes]
