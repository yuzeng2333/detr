...
class InvarHungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_ops = outputs["pred_ops"].flatten(0, 1).softmax(-1)  # [batch_size * num_queries, num_classes]
        out_eq = outputs["pred_eq"].flatten(0, 1).softmax  # [batch_size * num_queries, 3]

        tgt_ops = targets["ops"].flatten(0, 1)  # [total_num_ops]
        tgt_eq = targets["eq"].flatten(0, 1)  # [total_num_equations]
